...
.controller('ChatCtrl', function ($scope, $rootScope, $state, Message, $ionicActionSheet, $ionicPopup, $ionicScrollDelegate, $timeout, $interval) {
  ...
  $scope.$on('$ionicView.beforeLeave', function() {
    if (!$scope.input.message || $scope.input.message === '') {
      localStorage.removeItem('userMessage-' + $scope.currentChat.user.id);
    }
  });
  ...
  var fetchEarlierDone = function () {
    if(fetchEarlierTimer) {
      $timeout.cancel(fetchEarlierTimer);
      fetchEarlierTimer = null;
    }
    $scope.$broadcast('scroll.refreshComplete');
  }
  $scope.fetchEarlier = function () {
    ...
    fetchEarlierTimer = $timeout(fetchEarlierDone, 5000);
  }
  ...
  $scope.sendMessage = function(sendMessageForm) {
    var message = {
      ToUserName: $scope.currentChat.user.id,
